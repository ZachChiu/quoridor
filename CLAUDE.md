...
- **玩家色受兩個對比限制**：棋子與牆疊在 `#faf7f0` 上需 ≥3:1（WCAG 圖形元件），
  而三塊比分上的米白字需 ≥4.5:1。黃方因此比紅藍暗**兩**階（`#a06400`，
  OKLCH L 0.557）—— 黃本質上亮，不壓到這裡米白字只有 3.04:1，就得改用墨字，
  而三塊並排兩白一黑很難看。彩度因此是 0.123，是全站唯一低於 0.125 下限的值：
  在黃色相上「米白字過 4.5」與「彩度 ≥0.125」不可能同時成立，可讀性優先。
  也別為了拉彩度把色相推向橘（`#b15900` 彩度 0.142 但色相 55°），
  那對紅方的明度對比只有 1.06，盤面上兩種棋會混在一起。
